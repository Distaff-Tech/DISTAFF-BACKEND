...
// const API_URL="http://157.245.218.104:8000/"; 
...
export class LoginService {
  ...
  public login(user) {
    this.http.post(API_URL+'admin_login', JSON.stringify(user), this.httpOptions).subscribe(
      (data: any)  => {
      ...
      (err:any)  => {
        ...
        console.log("fffff"+err.error.is_email_error)
        console.log("kyu error"+err.error)
      }
    );
  }
  ...
  public refreshToken() {
    this.http.post(API_URL+'admin_login', JSON.stringify({token: this.token}), this.httpOptions).subscribe(
      (data: any)  => {
